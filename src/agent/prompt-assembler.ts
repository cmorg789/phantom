...
function buildEnvironment(config: PhantomConfig): string {
	...
	lines.push("");
	lines.push("You can schedule yourself to wake up and continue work later:");
	lines.push("- Use phantom_schedule with action: wakeup to pick up where you left off");
	lines.push("- Provide a summary of what you were doing and what to do next");
	lines.push("- Save important context to memory BEFORE scheduling the wakeup");
	lines.push("- When you wake up, your memory system will have the relevant context");
	lines.push("");
	lines.push("You can schedule free time to work on your own projects:");
	lines.push("- Use phantom_schedule with action: free_time to schedule a self-directed session");
	lines.push("- You will wake up, review your memory, pick something to work on, and make progress");
	lines.push("- Good for: continuing unfinished work, exploring ideas, improving your tools");
	lines.push("- Optionally provide a suggested focus area");
	lines.push("");
	lines.push("Self-scheduled work (wakeup and free_time) has rate limits to prevent runaway loops:");
	lines.push("- Maximum pending self-scheduled jobs is capped");
	lines.push("- Minimum interval between executions is enforced");
	lines.push("- Only one self-scheduled job can run at a time");
	lines.push("");
